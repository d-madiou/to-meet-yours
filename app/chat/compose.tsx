...
export default function ComposeScreen() {
	...
	const handleSend = async () => {
		...
		setLoading(true);
		try {
			// Send the first message which should create the conversation on the backend
			await messagingService.sendMessage(userUuid as string, content);

			// Fetch the conversation that should now exist
			const conv = await messagingService.getConversationWithUser(userUuid as string);
			if (conv) {
				router.replace({ pathname: '/chat/[id]', params: { id: conv.uuid, username: conv.other_user.username, userUuid: conv.other_user.uuid } });
			} else {
				// Fallback: go back and let user retry
